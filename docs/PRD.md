...
<tr><td>搜索与筛选</td><td>五维搜索（中/法/英名 + 关联名人 + 关联作品影视），每条建议暴露匹配原因</td><td>P2</td></tr>
</tbody>
</table>
...
搜索不是「按名字找地方」，而是**按人、按作品、按电影找地方**。用户记得的往往不是 POI 名，而是「萨特待过的那家咖啡馆」「《碟中谍》在巴黎拍的那栋楼」。

- **多维搜索**：匹配范围覆盖 POI 中文名、法文原名、英文名、关联名人、关联作品与影视，共五类。
- **暴露匹配原因**（差异化要点）：每条建议都标明「为什么这条会被搜出来」，并用小标签区分命中类型（地点名 / 名人 / 作品 / 影视 / 别名）。搜「萨特」时，前两条是名字里就含萨特的 POI，后三条是**他在那儿待过**——用户搜一个人名，拿到的是这个人在巴黎的活动轨迹。
- **别名重定向**：同一 POI 的中/法/英名互相可达，搜任一个命中同一条结果，不出现重复条目。
- **片名别名表**（必需）：大制作用代号报备（《碟中谍6》报备名为 `GEMINI`），需建立映射。搜 `GEMINI` 命中苏比兹公馆，匹配原因注明「片名代号 GEMINI 对应《碟中谍6》」。
- **交互**：输入即实时出建议无需回车；上下键移动、回车选中；点选后地图 flyTo 并弹出 POI 浮层；Esc 关下拉；有清空按钮。
- **与筛选并存**：搜索与时间轴、主题筛选互不清空。命中 POI 若被时间轴隐藏，临时显示并提示「这个地点不在当前时间范围内」。
- **筛选维度**：主题（文学/艺术/影视/音乐/历史）、时代（7 段）、街区。

![搜索建议下拉：搜「萨特」出 5 条，每条标明匹配原因与命中类型](./assets/search-suggest.png)
...
- [ ] POI ⟷ 主题线多对多互跳正常
- [ ] 主题线页可横向跳转其它 15 条线
- [x] 搜索支持中/法/英名、关联名人与关联影视，并展示匹配原因 *（原型已验证）*
- [x] 片名别名表生效（`GEMINI` 可检索到《碟中谍6》）*（原型已验证）*
